...
class SQLiteDB:
    ...
    def init_tables(self):
        ...
        self.exe("""CREATE TABLE IF NOT EXISTS bots(
            ...
            last_crash TEXT,
            created_at TEXT DEFAULT(datetime('now')),
            auto_restart_24h INTEGER DEFAULT 0   -- NEW: 1 = enabled
        )""")
        # Ensure column exists for existing databases (ignore error if already present)
        try:
            self.exe("ALTER TABLE bots ADD COLUMN auto_restart_24h INTEGER DEFAULT 0")
        except Exception:
    ...
    def add_bot(self, uid, name, path, entry='main.py', ft='py', tok='', sz=0, conf=''):
        ...
        try:
            r = self.db.bots.insert_one({
                ...
                'created_at': datetime.now().isoformat(),
                'auto_restart_24h': 0   # NEW: default off
            })
...
class WrappedSQLite(SQLiteDB):
    ...
    def add_bot(self, uid, name, path, entry='main.py', ft='py', tok='', sz=0, conf=''):
        return self.exe(
            "INSERT INTO bots(user_id,bot_name,file_path,entry_file,file_type,bot_token,file_size,detection_confidence,auto_restart_24h) VALUES(?,?,?,?,?,?,?,?,0)",

...
def main_menu_kb(uid):
    ...
    m.add(types.InlineKeyboardButton("📞 Contact Developer", url="https://t.me/chinkor"))
    return m
...
def bot_action_kb(bid, is_live, auto_restart=False):
    """Always show the auto-restart toggle button."""
    m = types.InlineKeyboardMarkup(row_width=2)
    if is_live:
        m.add(
    ...
    # Auto-restart toggle – always visible for all users
    toggle_text = "🔄 Auto Restart: ON" if auto_restart else "🔄 Auto Restart: OFF"
    m.add(types.InlineKeyboardButton(toggle_text, callback_data=f"bot_autorestart:{bid}"))
